...
// Some resource estimation code adapted from: https://github.com/sam-jaques/offline-quantum-period-finding and https://github.com/microsoft/grover-blocks

namespace cs
{
    class Estimates
    {
        // Code from microsoft grover-blocks updated to Microsoft.Quantum.Sdk/0.11.2004.2825
        // Later updated to Microsoft.Quantum.Sdk/0.18.2106148911
        ...
        public static String[] ProcessSimReturn<Qop>(QCTraceSimulator sim, string comment = "", bool full_depth = false, string suffix="")
        {
            //Console.WriteLine("ProcessSim started.");
            //foreach (var line in sim.ToCSV())
            //{
            //    //Console.WriteLine(line);
            //    File.WriteAllText("data.csv", line.ToString());
	    
            //}
            
            var returnvalue = new String[2];
            returnvalue[0] = DisplayCSV.GetHeader(full_depth) + ", size";
            returnvalue[1] = DisplayCSV.CSV(sim.ToCSV(), typeof(Qop).FullName);
        ...
        public static void MixByteIn<Qop>(string comment = "", int output_byte = 1)
        {
            var sim = getTraceSimulator(false);
            var res = QTestsAddon.Resource_TestMixColumnsIn.Run(sim, output_byte).Result;
            ProcessSim<Qop>(sim, comment);
            sim = getTraceSimulator(true);
            res = QTestsAddon.Resource_TestMixColumnsIn.Run(sim, output_byte).Result;
            ProcessSim<Qop>(sim, comment, true);
        }



        public static void CostCheapGroverOracle<Qop>(string comment = "", int Nr = 10, int Nk = 4, bool in_place_mixcolumn = true, bool free_swaps = true, string suffix = "")
        {

            var estimator1 = getTraceSimulator(false);   //T-depth estimator
            var estimator2 = getTraceSimulator(true);    //Full depth estimator

            var filename = typeof(Qop).FullName + ".csv";   
            var res1 = QTestsAddon.CostCheapGroverOracle.Run(estimator1, Nr, Nk, in_place_mixcolumn, free_swaps).Result;
            var res2 = QTestsAddon.CostCheapGroverOracle.Run(estimator2,Nr, Nk, in_place_mixcolumn, free_swaps).Result;

            var TDepthData    = ProcessSimReturn<Qop>(estimator1, "T depth stats", true)[1]; // Get the T-depth   
            var fullDepthData = ProcessSimReturn<Qop>(estimator2, "Full depth stats", true)[1];     // Get the full statistics

            var TDepthValues     = TDepthData.Split(",");
            var fullDepthValues  = fullDepthData.Split(",");
            var gateTotal = 0;             

            for (int i =1; i <= 5; i = i + 1)
            {
                gateTotal = gateTotal + int.Parse(fullDepthValues[i].Trim());
            }
            
            Console.WriteLine(fullDepthValues[11]);
            //if (File.Exists(filename) == false)
            //{
             //   Console.WriteLine("Creating file");
            //    File.Create(filename).Close();  
            //    File.AppendAllText(filename, "CNOT count,1-qubit Clifford count,T count,R count,M count,initial width,extra width,T depth,full depth,total gate count" + Environment.NewLine);
            //Console.WriteLine("CNOT count,1-qubit Clifford count,T count,R count,M count,initial width,extra width,T depth,full depth,total gate count,comment" + Environment.NewLine);
            Console.WriteLine("CNOT count,1-qubit Clifford count,T count,R count,M count,initial width,extra width,T depth,full depth,total gate count,comment");

            //}
            Console.WriteLine(fullDepthValues[1].Trim() + "," + fullDepthValues[2] + "," + fullDepthValues[3] + "," + fullDepthValues[4] + "," + fullDepthValues[5] + "," + fullDepthValues[9] + "," + fullDepthValues[10] + "," + TDepthValues[6] + "," + fullDepthValues[6] + "," + gateTotal.ToString() + "," + comment + Environment.NewLine);        

        }
    


        public static void CostSerialGroverOracle<Qop>(string comment = "", int Nr = 10, int Nk = 4, int r = 2, bool in_place_mixcolumn = true, bool free_swaps = true, string suffix = "")
        {
            ...
            var filename = typeof(Qop).FullName + ".csv";   
            var res1 = QTestsAddon.CostSerialGroverOracle.Run(estimator1, Nr, Nk, r, in_place_mixcolumn, free_swaps).Result;
            var res2 = QTestsAddon.CostSerialGroverOracle.Run(estimator2,Nr, Nk, r, in_place_mixcolumn, free_swaps).Result;
        ...
        public static void CostExpensiveSerialGroverOracle<Qop>(string comment = "", int Nr = 10, int Nk = 4, int r = 2, bool in_place_mixcolumn = true, bool free_swaps = true, string suffix = "")
        {
            ...
            var filename = typeof(Qop).FullName + ".csv";   
            var res1 = QTestsAddon.CostExpensiveSerialGroverOracle.Run(estimator1, Nr, Nk, r, free_swaps).Result;
            var res2 = QTestsAddon.CostExpensiveSerialGroverOracle.Run(estimator2,Nr, Nk, r, free_swaps).Result;
